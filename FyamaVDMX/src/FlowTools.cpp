...
void FlowTools::setup(){
	...
	drawWidth = SCREEN_WIDTH / 8;
	drawHeight = SCREEN_HEIGHT / 8;
...
//--------------------------------------------------------------
/*
void FlowTools::keyPressed(int key){
	switch (key) {
		case 'G' :
...
//--------------------------------------------------------------
void FlowTools::mouseDragged(int x, int y, int button) {
	ofVec2f mouse;
...
//--------------------------------------------------------------
void FlowTools::mouseMoved(int x, int y){
	ofVec2f mouse;
	mouse.set(x / (float)ofGetWindowWidth(), y / (float)ofGetWindowHeight());
	lastMouse.set(mouse.x, mouse.y);
}
