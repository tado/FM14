...
string StFftCubeDistort::getName(){
    return "StFftCubeDistort";
}

void StFftCubeDistort::setup(){
    gui = new ofxUICanvas();
    ...
    gui->loadSettings("StFftCubeDistort.xml");
...
void StFftCubeDistort::draw(){
    ...
    float controlHue;
    controlHue = ofMap(app->oscControl->controlVal[3], 0, 127, 0, 0.55);
...
void StFftCubeDistort::guiEvent(ofxUIEventArgs &e){
    ...
    if(name == "SAVE SETTINGS"){
        gui->saveSettings("StFftCubeDistort.xml");
    }
}
