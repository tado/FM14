...
string StFftRibbonCool::getName(){
    return "StFftRibbonCool";
}

void StFftRibbonCool::setup(){
    gui = new ofxUICanvas();
    ...
    gui->loadSettings("StFftRibbonCool.xml");
...
void StFftRibbonCool::guiEvent(ofxUIEventArgs &e){
    string name = e.widget->getName();
    if(name == "SAVE SETTINGS"){
        gui->saveSettings("StFftRibbonCool.xml");
    }
}
...
void StFftRibbonCool::createMesh(){
    ...
    for (int i = 0; i < mesh.getVertices().size(); i++) {
        ...
        int hue = ofMap(i, 0, mesh.getVertices().size()-1, 120, 140);
...
void StFftRibbonCool::stateEnter(){
    createMesh();
}

void StFftRibbonCool::stateExit(){
    ribbons.clear();
    currentVertex.clear();
